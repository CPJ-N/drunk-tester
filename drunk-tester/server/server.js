...
{
    let items = await runQuery(`SELECT * from c where c.username=\"${username}\"`,container);
...
async function upsertProgress(body,sessionid,container)
{
    ...
    let items = await runQuery(`SELECT * from c where c.type=\"quizattempt\" and c.sessionid = \"${sessionid}\"`,container);
...
app.post('/adduser', asyncHandler(async (req, res, next) => {
    console.log(`adding user with  ${req.query.username} and ${req.query.password}`);
...
app.get('/authenticate', asyncHandler(async (req, res, next) => 
{
    ...
    let query = `SELECT * from c where c.type=\"user\" and c.username=\"${req.query.username}\" and c.passwordHash=\"${createPasswordHash(req.query.password)}\"`;
    let items = await runQuery(query, container);
    ...
    console.log(`trying to log in using ${req.query.username} and ${req.query.password} `);
    if (items.length === 1)
    {
...
app.post('/savequizprogress/:sessionid', asyncHandler(async (req, res, next) =>
{
    try {

        console.log(`saving progress for ${req.params.sessionid} with body of ${req.body}`);
...
//      When successful will return HTTP 200 with OK
//      When failed, HTTP 400 is returned
app.get('/readquiz/:sessionid', asyncHandler(async (req, res, next) =>
{
    try {
